...
//
//
// An XScreenSaver compatible BOINC screensaver for Unix/X11.
//
// To use this screensaver, add the following to the 'programs'
...
private:
    ...
    // convert std::string to xcb_char2b_t*
    ...
    ): con(connection), scr(screen), client_win(0)
    {
        ...
        if (windowed) {
            ...
            cookie = xcb_create_window_checked(
                con, XCB_COPY_FROM_PARENT, win, parent, 0, 0, width, height, 0,
        ...
        cookie = xcb_open_font_checked(
            con, font, font_name.size(), font_name.c_str()
        );
        error = xcb_request_check(con, cookie);
        if (error) {
        ...
        case XCB_EXPOSE:
            {
                ...
                if (!expose_event->count
                    && window
                    && window->get_window_id() == expose_event->window
                ) {
                    window->redraw();
                }
                break;
...
int main(int argc, char *argv[]) {
    ...
    for (int n = 0; n < 10; n++) {
        ...
        xcb_get_property_cookie_t cookie = xcb_get_property(
            con, 0, screen->root, reply0->atom, XCB_ATOM_WINDOW, 0,
             std::numeric_limits<uint32_t>::max()
        );

        xcb_generic_error_t    *error;
        xcb_get_property_reply_t *reply = xcb_get_property_reply(con, cookie, &error);
        if (error) {
        ...
        for (unsigned int c = 0; c < reply->length; c++) {
            ...
            cookie = xcb_get_property(
                con, 0, clients[c], XCB_ATOM_WM_COMMAND, XCB_ATOM_STRING,
            ...
            // check WM_CLASS
            cookie = xcb_get_property(
                con, 0, clients[c], XCB_ATOM_WM_CLASS, XCB_ATOM_STRING,
                0, std::numeric_limits<uint32_t>::max()
            );
            reply2 = xcb_get_property_reply(con, cookie, &error);
            if (!error) {    // ignore errors 
                ...
                if (pos == std::string::npos) {
                    executable = graphics_cmd;
                } else {
                    executable = graphics_cmd.substr(pos + 1);
                }
