...
if [ ! -e "${CURL_FLAGFILE}" -a $BUILD_WITH_VCPKG = "no" ]; then
    ...
    if [ -n "$CONFIGURE" ]; then
        ./configure --host=x86_64-linux --prefix="$TCINCLUDES" --libdir="$TCINCLUDES/lib" --disable-shared --enable-static --with-random=/dev/urandom 1>$STDOUT_TARGET --with-openssl --with-nghttp2
    fi
    if [ $VERBOSE = "no" ]; then
