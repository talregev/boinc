...
class StorageFragment : Fragment() {
    private lateinit var listAdapter: StorageListAdapter
    private val data: MutableList<StorageListData> = ArrayList()
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        ...
        listAdapter = StorageListAdapter(activity, binding.projectsList, R.id.projects_list, data)
        return binding.root
    }
    ...
    private fun updateData(latestRpcProjectsList: List<Project>, acctMgrInfo: AcctMgrInfo,
        ...
        if (mgrIndex < 0) { // no manager present until now
            ...
            if (acctMgrInfo.isPresent) {
                ...
                data.add(StorageListData(null, acctMgrInfo, null))
    ...
    inner class StorageListData(
        ...
        val projectsListClickListener = View.OnClickListener {
            ...
            val controls: MutableList<StorageControl> = ArrayList()
            if (isMgr) {
                dialogBinding.title.setText(R.string.projects_control_dialog_title_acctmgr)
                controls.add(StorageControl(listEntry, VISIT_WEBSITE))
                controls.add(StorageControl(listEntry, RpcClient.MGR_SYNC))
                controls.add(StorageControl(listEntry, RpcClient.MGR_DETACH))
            } else {
                dialogBinding.title.setText(R.string.projects_control_dialog_title)
                controls.add(StorageControl(listEntry, VISIT_WEBSITE))
                if (!projectTransfers.isNullOrEmpty()) {
                    controls.add(StorageControl(listEntry, RpcClient.TRANSFER_RETRY))
                }
                controls.add(StorageControl(listEntry, RpcClient.PROJECT_UPDATE))
                if (project!!.suspendedViaGUI) {
                    controls.add(StorageControl(listEntry, RpcClient.PROJECT_RESUME))
                } else {
                    controls.add(StorageControl(listEntry, RpcClient.PROJECT_SUSPEND))
                }
                ...
                if (isShowAdvanced) {
                    if (project!!.doNotRequestMoreWork) {
                        controls.add(StorageControl(listEntry, RpcClient.PROJECT_ANW))
                    } else {
                        controls.add(StorageControl(listEntry, RpcClient.PROJECT_NNW))
                    }
                    controls.add(StorageControl(listEntry, RpcClient.PROJECT_RESET))
                }
                if (!project!!.attachedViaAcctMgr) {
                    controls.add(StorageControl(listEntry, RpcClient.PROJECT_DETACH))
                }
            }

            // list adapter
            dialogBinding.options.adapter = StorageControlsListAdapter(activity!!, controls)
    ...
    inner class StorageControl(var data: StorageListData, var operation: Int) {
    ...
    private suspend fun performProjectOperationAsync(data: StorageListData, operation: Int) = coroutineScope {
    ...
    private fun performProjectOperation(data: StorageListData, operation: Int): Boolean {
        try {
