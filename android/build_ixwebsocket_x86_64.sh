...
if [ ! -e "${IXWEBSOCKET_FLAGFILE}" -a  $BUILD_WITH_VCPKG = "no" ]; then
    mkdir -p "$IXWEBSOCKET/build-x86_64"
    cd "$IXWEBSOCKET/build-x86_64"
    echo "===== building IXWEBSOCKET for x86-64 from $PWD ====="
    ...
    echo "\e[1;32m===== IXWEBSOCKET for x86-64 build done =====\e[0m"
fi
