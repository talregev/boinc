...
function findWasmBinary() {
  return locateFile("app_gpu.wasm");
}
...
function __asyncjs__wasm_gpu_init(n, k) {
  return Asyncify.handleAsync(async () => {
    ...
    for (let i = 0; i < n; i++) input[i] = (i * 2654435761 + 1) >>> 0;
    const bytes = n * 4;
    ...
    const parBuf = device.createBuffer({
      size: 16,
...
function __asyncjs__wasm_gpu_dispatch() {
  return Asyncify.handleAsync(async () => {
    const g = Module.gpu;
    ...
    pass.dispatchWorkgroups(Math.ceil(g.n / 64));
    pass.end();
    g.device.queue.submit([enc.finish()]);
    await g.device.queue.onSubmittedWorkDone();
  });
}
function __asyncjs__wasm_gpu_readback() {
  return Asyncify.handleAsync(async () => {
    const g = Module.gpu;
    const enc = g.device.createCommandEncoder();
...
function wasm_gpu_info(buf, len) {
  const s = (Module.gpu && Module.gpu.info) || "unknown";
  stringToUTF8(s, buf, len);
}
function wasm_app_finish(status) {
  var files = {};
  try {
    FS.readdir(".").forEach(function (n) {
      if (n === "." || n === "..") return;
      try {
        if (FS.isFile(FS.stat(n).mode)) files[n] = FS.readFile(n);
      } catch (e) {}
    });
  } catch (e) {}
  postMessage({ boincFinish: status, files });
}
function wasm_shm_alloc(size) {
  if (!Module.boincShm)
    Module.boincShm = new Uint8Array(new SharedArrayBuffer(size));
}
function wasm_shm_get(off, outPtr) {
  ...
  HEAPU8[outPtr + i] = 0;
  Atomics.store(u8, off, 0);
  return 1;
}
function wasm_shm_send(off, msgPtr, overwrite) {
  var u8 = Module.boincShm;
  if (!overwrite && Atomics.load(u8, off) !== 0) return 0;
  var i = 0;
  for (; i < 1022; i++) {
    var c = HEAPU8[msgPtr + i];
    u8[off + 1 + i] = c;
    if (c === 0) break;
  }
  u8[off + 1 + i] = 0;
  Atomics.store(u8, off, 1);
  return 1;
}
var _main,
  _malloc,
...
var wasmImports = {
  __asyncjs__wasm_gpu_dispatch,
  __asyncjs__wasm_gpu_init,
  __asyncjs__wasm_gpu_readback,
