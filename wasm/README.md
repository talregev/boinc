...
|---|---:|---:|
| CPU — native, 1 thread (`native_bench.c`) | ~637 | 1.2× |
| CPU — native, 12 threads (`native_bench.c`) | ~3,696 | ~7× |
| **CPU — wasm, 1 thread** (in-browser, the client's own benchmark) | **~520** | 1× |
| **GPU — WebGPU** (in-browser, through the BOINC pipeline) | **~33,000** | **~64×** |

The native rows are a **committed, reproducible** compiled-C benchmark —
`wasm/spikes/spikeB-webgpu/native_bench.c` (build + run: `./build_native_bench.sh && ./native_bench`)
— running the identical kernel and asserting the same `0x27a723a9` checksum (earlier figures came from
a Node oracle; this replaces that approximation).

Takeaways: single-thread wasm is ~0.82× native (the SIMD-enabled build is close to native for this
